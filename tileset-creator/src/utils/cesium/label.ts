    ...
    this.xAxisEntity = this.viewer.entities.add({
      polyline: {
        positions: new CallbackProperty(() => {
          const pos = this.pinEntity.position?.getValue(
            this.viewer.clock.currentTime,
          );
          if (!pos) return [];

          const east = Cartesian3.add(
            pos,
            new Cartesian3(shortAxisLength, 0, 0),
            new Cartesian3(),
          );
          return [pos, east];
        }, false),
    ...
    this.yAxisEntity = this.viewer.entities.add({
      polyline: {
        positions: new CallbackProperty(() => {
          ...
          const north = Cartesian3.add(
            pos,
            new Cartesian3(0, shortAxisLength, 0),
            new Cartesian3(),
          );
          return [pos, north];
        }, false),
    ...
    this.zAxisEntity = this.viewer.entities.add({
      polyline: {
        positions: new CallbackProperty(() => {
          ...
          const up = Cartesian3.add(
            pos,
            new Cartesian3(0, 0, shortAxisLength),
            new Cartesian3(),
          );
          return [pos, up];
        }, false),
