...
import {
  ...
  computeParametersFromTransform,
} from "../utils/cesium/transforms";
...
function MapInfoCustom({ mapInfo, viewer }: PropsType) {
  ...
  const [params, setParams] = useState(() => {
    ...
    if (
      ...
      mapInfo.tile instanceof Cesium.Cesium3DTileset
    ) {
      ...
      if (
        ...
        initialMatrix = Cesium.Matrix4.multiply(
          model.modelMatrix,
    ...
    const pos = Cesium.Matrix4.getTranslation(
      initialMatrix,
  ...
  const handleSaveTileset = async () => {
    ...
    try {
      ...
      if (!putResponse.ok) {
        throw new Error(
          `Failed to save tileset transform: ${putResponse.statusText}`,
        );
      }

      setSaveStatus({
        type: "success",
        message: "Tileset transform saved successfully!",
      });
    } catch (error: any) {
      setSaveStatus({
        type: "danger",
